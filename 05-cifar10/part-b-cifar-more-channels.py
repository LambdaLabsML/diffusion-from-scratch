...
class Model(torch.nn.Module):
    def __init__(self, image_channels=3, model_channels=32):
        ...
        self.enc1_1 = ResnetBlock(model_channels, model_channels, embed_dim)
        self.enc1_2 = ResnetBlock(model_channels, model_channels * 2, embed_dim)
        self.downconv1 = Downsample(model_channels * 2, model_channels * 2)
        self.enc2_1 = ResnetBlock(model_channels * 2, model_channels * 2, embed_dim)
        self.enc2_2 = ResnetBlock(model_channels * 2, model_channels * 4, embed_dim)
        self.downconv2 = Downsample(model_channels * 4, model_channels * 4)
        self.bottleneck_1 = ResnetBlock(model_channels * 4, model_channels * 4, embed_dim)
        self.bottleneck_2 = ResnetBlock(model_channels * 4, model_channels * 4, embed_dim)
        self.upconv2 = Upsample(model_channels * 4, model_channels * 4)
        self.dec2_1 = ResnetBlock(model_channels * 8, model_channels * 4, embed_dim)
        self.dec2_2 = ResnetBlock(model_channels * 4, model_channels * 2, embed_dim)
        self.upconv1 = Upsample(model_channels * 2, model_channels * 2)
        self.dec1_1 = ResnetBlock(model_channels * 4, model_channels * 2, embed_dim)
        self.dec1_2 = ResnetBlock(model_channels * 2, model_channels, embed_dim)
...
def train(batch_size=128, epochs=80, lr=1e-3, model_channels=32):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    noise_scheduler = NoiseScheduler().to(device)
    model = Model(model_channels=model_channels).to(device)
    ...
    torch.save(model.state_dict(), 'part-b-cifar-more-channels-model.pth')

def test(model_channels=32):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    noise_scheduler = NoiseScheduler().to(device)
    model = Model(model_channels=model_channels).to(device)
    model.load_state_dict(torch.load('part-b-cifar-more-channels-model.pth', weights_only=True))
    model.eval()
    ...
    grid.save("part-b-cifar-more-channels-output.png")
    print("Image saved as part-b-cifar-more-channels-output.png")
