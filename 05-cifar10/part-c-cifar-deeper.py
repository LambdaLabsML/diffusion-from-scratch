...
class Model(torch.nn.Module):
    def __init__(self, num_steps=1000, ch=16):
...
def train(batch_size=128, epochs=80, lr=1e-3):
    ...
    torch.save(model.state_dict(), 'cifar-deeper-model.pth')

def test():
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    noise_scheduler = NoiseScheduler(steps=1000, beta_start=1e-4, beta_end=0.02).to(device)
    model = Model().to(device)
    model.load_state_dict(torch.load('cifar-deeper-model.pth', weights_only=True))
    model.eval()
    ...
    grid.save("cifar-deeper-output.png")
    print("Image saved as cifar-deeper-output.png")
